...
df_sweeps$structure_str = gsub('_', 'x', df_sweeps$structure)
...
df_summary$structure_str = gsub('_', 'x', df_summary$structure)
df_summary$structure_factor = factor(df_summary$structure_str, levels = c(
  '60x60',
  '30x120',
  '15x240',
  '4x900',
...
#ggplot(df_sweeps, aes(x = structure_factor, y = Generation)) + 
#  geom_boxplot(outlier.shape = NA) + 
...
ggplot(df_sweeps, aes(x=sweep_num, y = Generation, group=rep)) + 
  ...
  facet_grid(rows = vars(structure_factor))
