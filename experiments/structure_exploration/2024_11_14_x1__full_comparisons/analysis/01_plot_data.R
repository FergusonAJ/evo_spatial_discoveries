...
ggplot(df_raw[df_raw$structure == 'linear_chain',], aes(x = discovery_site_idx_first)) + 
  geom_histogram(binwidth = 36) + 
  facet_grid(rows = vars(reward), cols = vars(fitness_step))

ggplot(df_plot[df_plot$structure == 'star',], aes(x = discovery_site_idx_first)) + 
  geom_histogram(binwidth = 3) + 
  facet_grid(rows = vars(reward), cols = vars(fitness_step))

ggplot(df_plot[df_plot$structure == 'cycle',], aes(x = discovery_site_idx_first)) + 
  geom_histogram(binwidth = 36) + 
  facet_grid(rows = vars(reward), cols = vars(fitness_step))

df_raw$new_trial_seed = paste0(df_raw$rep_id, '_', df_raw$trial_id)

df_summary_2 = dplyr::group_by(df_plot, structure, reward, fitness_step, structure_factor) %>%
  dplyr::summarize(unique_seeds = n_distinct(new_trial_seed), 
                   reward_factor = dplyr::first(reward_factor), 
                   fitness_step_factor = dplyr::first(fitness_step_factor)
                   )

df_summary_2$first_discovery_frac = df_summary$unique_seeds / 10^4
