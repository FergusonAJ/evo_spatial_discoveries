...
df_phylo$structure_abbr = abbreviation_map[df_phylo$structure_str]
df_phylo$structure_abbr_factor = factor(df_phylo$structure_abbr, levels = c(
  'wm',
  'tg',
  'lc', 
  'cyc', 
...
ggp_tmp = ggplot(df_final[df_final$landscape == 'Single gradient',], aes(x = structure_abbr_factor, y = mean_pairwise_distance, color = structure_factor)) + 
    geom_boxplot() + 
    geom_jitter(alpha = 0.5) +
