...
output_file=../combined_phylogenetic_data.csv
header_copied=0
...
do
    ...
    echo "Scraping phylo for rep: ${dir_name}"
    SEED=$(( SEED_BASE + (rep_seed * 10000) ))

    grep -P "^\d+00," ${full_name}/phylogenetic_data.csv | sed -E "s/^/${dir_name},/" >> ${output_file}
done
