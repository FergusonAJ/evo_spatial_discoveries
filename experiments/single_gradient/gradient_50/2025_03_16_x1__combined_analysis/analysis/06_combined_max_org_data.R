rm(list = ls())

df_combined = NA
gradient = 0.01
max_fitness = 50

for(dir in list.dirs('../..', recursive = F)){
  print(dir)
  exp_id = strsplit(dir, '_')[[1]][4]
  if(grepl('x', exp_id)){
    print('Skipping!')
    next
  }
  structure = strsplit(dir, '__')[[1]][-1]
  print(structure)
  df_dir = read.csv(paste0(dir, '/data/combined_max_org_over_time.csv'))
  df_dir$gradient = gradient
  df_dir$structure = structure
  df_dir$max_fitness = max_fitness
  if(is.data.frame(df_combined)){
    df_combined = rbind(df_combined, df_dir)
  } else {
    df_combined = df_dir
  }
}

write.csv(df_combined, '../data/combined_max_org_over_time_data.csv', row.names = F)
